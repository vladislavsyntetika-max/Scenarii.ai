...
import { Sparkles, Trash2, ChevronDown, ChevronUp, BookOpen, Loader2, AlertCircle, PenTool } from 'lucide-react';
...
function ScriptCard({ script, onDelete }: { script: SavedScript; onDelete: (id: number) => void }) {
    ...
    >
      ...
      >
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2 mb-1">
            <span className="text-[11px] font-bold tracking-widest text-primary uppercase">{script.niche}</span>
            <span className="text-[11px] text-muted-foreground">·</span>
            <span className="text-[11px] text-muted-foreground">{date}</span>
          </div>
          <p className="text-sm font-semibold leading-snug line-clamp-2 text-foreground/90">"{script.hook}"</p>
        </div>
      ...
      <AnimatePresence initial={false}>
        {expanded && (
          ...
          >
            <div className="px-4 sm:px-5 pb-5 space-y-5 border-t border-border/40 pt-4">
              {/* Idea */}
              ...
              <div className="space-y-1.5">
                ...
                <p className="font-bold text-base leading-snug">"{script.hook}"</p>
              </div>
              ...
              <div className="space-y-1.5">
                ...
                <p className="font-semibold italic text-foreground/90">"{script.cta}"</p>
              </div>
            </div>
          </motion.div>
        )}
...
export default function MyScripts() {
  ...
  useEffect(() => {
    fetch('/api/scripts')
      .then(r => r.json())
      .then((data: SavedScript[]) => { setScripts(data); setLoading(false); })
      .catch(() => { setError('Не удалось загрузить сценарии.'); setLoading(false); });
  }, []);
        ...
        <main className="flex-1 flex flex-col gap-4">
          ...
          {!loading && !error && scripts?.length === 0 && (
            ...
            >
              ...
              <p className="text-muted-foreground text-sm">Сохранённых сценариев пока нет.<br />Сгенерируйте и сохраните первый!</p>
              <button
                onClick={() => navigate('/')}
